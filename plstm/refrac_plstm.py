...
class RefracPLSTMTimeGate(object):
    """
    """
    def __init__(self,
...
class RefracPLSTMLayer(MergeLayer):
    r"""
    """
    ...
    def __init__(self, incoming, time_input, num_units,
        ...
        super(RefracPLSTMLayer, self).__init__(incomings, **kwargs)
    ...
    def get_output_for(self, inputs, deterministic=False, **kwargs):
        ...
        def step_masked(input_n, time_input_n, mask_n, cell_previous, hid_previous, refrac_end_prev, *args):
            ...
            return [cell, hid, refrac_end_new]
        ...
        non_seqs = [W_hid_stacked, self.period_timegate]
        ...
        # PLSTM:
        refrac_end_init = T.dot(ones*0., self.cell_init)
        if self.unroll_scan:
            # Retrieve the dimensionality of the incoming layer
            input_shape = self.input_shapes[0]
            # Explicitly unroll the recurrence instead of using scan
            cell_out, hid_out, refrac_out = unroll_scan(
                fn=step_fun,
                sequences=sequences,
                outputs_info=[cell_init, hid_init, refrac_end_init],
            ...
            cell_out, hid_out, refrac_out = theano.scan(
                fn=step_fun,
                sequences=sequences,
                outputs_info=[cell_init, hid_init, refrac_end_init],
                go_backwards=self.backwards,
